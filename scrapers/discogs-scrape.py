...
dbconn = psycopg2.connect(host='fernando.nluken.com', dbname='music_analysis',
                          user='nluken', password='arcadia22')
cur = dbconn.cursor()
cur2 = dbconn.cursor()
print("connected to db\n")

d = discogs_client.Client('DiscogsFormatDataAnalyzer/0.1', user_token='gAHNhkitFMXWlVoRKZGVZpgpDojaDVznMDvBYJNt')

#get the albums table
cur.execute("SELECT * FROM albums WHERE genre IS NULL order by title desc;")
for album in cur:
  ...
  ## we need to sleep to avoid rate limits
    ...
    cur2.execute("UPDATE albums SET genre = 'None' WHERE id = '%s'" % album[0])
    dbconn.commit()
  time.sleep(2)
